...
def ejecucion_supervisado():
  ...
  x_escalado = escalar_datos(X)
  X_train, X_test, y_train, y_test = dividir_datos(x_escalado, y)
...
def ejecucion_no_supervisado():
  ...
  modelo = entrenar_modelo_kmeans(x_normalizado)
  etiquetas = asignar_etiquetas(modelo, x_normalizado)
  visualizar_clusters(x_normalizado, etiquetas)
  return
